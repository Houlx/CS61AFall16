...
def free_bacon(opponent_score):
    ...
    # BEGIN PROBLEM 2
    return opponent_score % 10 + 1 if opponent_score % 10 > opponent_score // 10 else opponent_score // 10 + 1
    # END PROBLEM 2
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls > 0:
        ...
        return min(25 - num_rolls, result)
    else:
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 5
    def turn(strategy_player, score_player, score_opponent):
        player_rolls = strategy_player(score_player, score_opponent)
        nonlocal dice_swapped
        if player_rolls == -1:
            dice_swapped = not dice_swapped
            return 1
        else:
            dice_type = select_dice(score_player, score_opponent, dice_swapped)
            return take_turn(player_rolls, score_opponent, dice_type)

    while score0 < goal and score1 < goal:
        if player == 0:
            score0 += turn(strategy0, score0, score1)
        else:
            score1 += turn(strategy1, score1, score0)
